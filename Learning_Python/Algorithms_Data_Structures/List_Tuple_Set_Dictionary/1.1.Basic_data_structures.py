...
# In this example we actually deleted only 2 itmes from the list.Method worked, but when it started to iterate the list
# from the first element '1',  index and elements moved so the next element it saw was not '2' but '3' and after that
# due to the same reason the loop finished it's work and didn't see '4'
# .remove deals with elemetns not indexes.
for i, item in enumerate(list_2):
    list_2.remove(item)
print(list_2)

# same situation as with .remove above.
...
# Here deleting(removing) procees was carried out from the list,
# while loop moved through it's slice (virtual copy of the list)
